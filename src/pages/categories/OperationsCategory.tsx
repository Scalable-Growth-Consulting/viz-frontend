...
import { ArrowLeft, Settings, Crown, Sparkles } from 'lucide-react';
...
const OperationsCategory: React.FC = () => {
  const { user } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-pink-50/30 dark:from-viz-dark dark:to-black">
      ...
      <main className="container mx-auto px-4 py-8 sm:py-12 max-w-7xl space-y-12">
        <div className="space-y-6">
          ...
          <div className="relative overflow-hidden rounded-3xl border border-slate-200/60 bg-white/80 backdrop-blur-sm shadow-2xl">
            <div className="absolute inset-0 bg-gradient-to-br from-purple-500/10 via-pink-100/20 to-rose-100/30" />
            <div className="relative z-10 px-8 sm:px-12 py-12 md:py-16 space-y-6">
              <div className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-purple-500 to-pink-600 px-5 py-2 text-xs font-bold uppercase tracking-wider text-white shadow-lg">
              ...
              <p className="text-xl text-slate-600 leading-relaxed max-w-4xl">
                Operational efficiency agents—IIA for inventory intelligence and DUFA for demand forecasting and optimization.
              </p>
            </div>
          </div>
        </div>
        ...
        <section className="space-y-8">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl md:text-3xl font-bold text-viz-dark dark:text-white">
              All Operations Agents ({operationsAgents.length})
            </h2>
          </div>
                ...
                <Link to={agent.route} className="group block h-full">
                  <div className="relative h-full rounded-2xl border border-slate-200/60 bg-white/90 backdrop-blur p-6 shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300">
                    ...
                    <div className="space-y-2">
                      ...
                      <h3 className="text-2xl font-bold text-viz-dark group-hover:text-purple-600 transition-colors">
                        {agent.name}
                      </h3>
                    </div>
          ...
          {operationsAgents.length === 0 && (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-white/60 px-8 py-12 text-center">
              ...
              <p className="text-lg text-slate-600 font-medium">
                No operations agents available yet.
              </p>
