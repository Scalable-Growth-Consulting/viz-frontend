...
import { Brain, Target, Sparkles, Globe } from 'lucide-react';
...
const MIACategory: React.FC = () => {
  const navigate = useNavigate();

  const miaAgents = AGENT_LIST.filter(agent => 
    agent.id === 'mia-core' || 
    agent.id === 'creative-labs' || 
    agent.id === 'brandlenz'
  );

  const iconMap: Record<string, any> = {
    'mia-core': Target,
    'creative-labs': Sparkles,
    'brandlenz': Globe,
  };
      ...
      <main className="flex-1">
        ...
        >
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute -top-40 -right-40 w-96 h-96 bg-gradient-to-r from-purple-500 to-violet-600 opacity-20 blur-3xl rounded-full" />
            <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-gradient-to-r from-purple-500 to-violet-600 opacity-10 blur-3xl rounded-full" />
          </div>
          ...
          <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
            <div className="max-w-4xl mx-auto text-center space-y-6">
              ...
              >
                <div className="p-4 bg-gradient-to-r from-purple-500 to-violet-600 rounded-3xl shadow-2xl">
              ...
              >
                Transform your marketing with AI-powered intelligence, creative automation, and brand monitoring capabilities.
              </motion.p>
            </div>
          </div>
        </motion.div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {agent.tags.map((tag, i) => (
                          <span
                            key={i}
                            className="px-3 py-1 bg-purple-500/10 text-purple-600 dark:text-purple-400 text-xs font-semibold rounded-full"
                          >
                            {tag}
                          </span>
                        ))}
