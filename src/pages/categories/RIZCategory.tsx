...
const RIZCategory: React.FC = () => {
  const navigate = useNavigate();

  const rizAgents = AGENT_LIST.filter(agent => 
    agent.id === 'dufa' || 
    agent.id === 'inventory-insight'
  );

  const iconMap: Record<string, any> = {
    'dufa': TrendingUp,
    'inventory-insight': Package,
  };
      ...
      <main className="flex-1">
        ...
        >
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute -top-40 -right-40 w-96 h-96 bg-gradient-to-r from-viz-accent to-blue-600 opacity-20 blur-3xl rounded-full" />
            <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-gradient-to-r from-viz-accent to-blue-600 opacity-10 blur-3xl rounded-full" />
          </div>
          ...
          <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
            <div className="max-w-4xl mx-auto text-center space-y-6">
              ...
              >
                <div className="p-4 bg-gradient-to-r from-viz-accent to-blue-600 rounded-3xl shadow-2xl">
              ...
              >
                Optimize inventory, forecast demand, and maximize retail performance with AI-powered intelligence.
              </motion.p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {rizAgents.map((agent, index) => {
                const Icon = iconMap[agent.id] || TrendingUp;
                return (
                  <motion.div
                    key={agent.id}
                    initial={{ opacity: 0, y: 20 }}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {agent.tags.map((tag, i) => (
                          <span
                            key={i}
                            className="px-3 py-1 bg-viz-accent/10 text-viz-accent dark:text-blue-400 text-xs font-semibold rounded-full"
                          >
                            {tag}
                          </span>
                        ))}
