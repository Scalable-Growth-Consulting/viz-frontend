...
import { MailIcon, GithubIcon, LogInIcon, Loader2 } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Simulate API call - in a real app, you'd call your auth API
  ...
  return (
    <div className="flex min-h-[80vh] flex-col items-center justify-center px-6 py-12">
      <div className="bg-viz-dark p-4 rounded-lg mb-4 animate-fade-in shadow-lg">
        <span className="text-4xl font-bold tracking-tight text-white">V</span>
      </div>
      <h1 className="text-3xl font-bold tracking-tight text-viz-dark dark:text-white mb-2 animate-fade-in">Welcome to Viz</h1>
      <p className="text-sm text-viz-text-secondary mb-8 animate-fade-in">Business Intelligence AI</p>
      ...
      <div className="w-full max-w-md">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" className="w-full bg-viz-accent hover:bg-viz-accent-light" disabled={isLoading}>
            {isLoading ? (
              <>
