...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      // Position elegantly at bottom-right with spacing and gap
      "fixed z-[100] bottom-6 right-6 flex flex-col gap-3 w-[min(92vw,420px)] max-h-screen",
      className
    )}
    {...props}
  />
   ...
   data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-bottom-4",
  {
    variants: {
      variant: {
        default: "",
        destructive:
          "destructive group border-red-300/60 bg-red-50/90 text-red-900 dark:border-red-400/30 dark:bg-red-900/30 dark:text-red-100",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    className={cn("text-[13px] leading-5 opacity-90 text-slate-600 dark:text-slate-200/90", className)}
    {...props}
  />
))
